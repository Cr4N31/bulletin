  ...
  if (!habit) {
    return (
      <SafeAreaView className="flex-1 bg-[#F8F8F8] items-center justify-center">
        <Text className="text-black/30">Loading...</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    ...
    >
      <View className="px-6 py-5 flex-row items-center gap-3">
        ...
        <Text className="text-2xl font-bold flex-1">{habit.title}</Text>
        {habit.isPriority && <Ionicons name="star" size={20} color="#000" />}
      </View>
      ...
      <View className="px-6 mt-8">
        <Text className="text-lg font-bold mb-4">Last 30 days</Text>
      ...
      <TouchableOpacity
        ...
        className={`mx-6 mt-10 h-14 rounded-2xl items-center justify-center flex-row gap-2 ${
          isDoneToday ? "bg-black/5 border border-black/10" : "bg-black"
        }`}
      >
        <Ionicons
          name={isDoneToday ? "checkmark-circle" : "checkmark-circle-outline"}
          size={20}
          color={isDoneToday ? "#000" : "#fff"}
        />
        <Text
          className={`font-semibold text-base ${isDoneToday ? "text-black" : "text-white"}`}
        >
